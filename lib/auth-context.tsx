...
export function AuthProvider({children}: { children: React.ReactNode }) {
    ...
    const getUser = async () => {
        try {
            ...
            console.log("getUser error:", error instanceof Error ? error.message : error);
            setUser(null);
        } finally {
    ...
    const signUp = async (email: string, password: string) => {
        try {
            ...
            // Try to create session, if one exists already, it will fail
            // but we can still get the current user
            try {
